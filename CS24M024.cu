...
__global__ void CalculateInvertedGrayScale(long int *d_red, long int *d_green, long int *d_blue,
long int *d_T1,long int m,long int n)
{
    ...
    if(id<(m*n))
    {
        ...
        long int j=id%n;
        long int m_i = m - 1 - i;
        double temp=((d_red[i * n + j]+d_green[i * n + j]+d_blue[i * n + j])/3.0);
        d_T1[ m_i * n + j]=(long int)floor(temp);
    }
}

__global__ void CalculateThomasTransformation(long int *d_red, long int *d_green, long int *d_blue,
long int *d_T2,long int m,long int n)
{
    long int id=blockIdx.x*blockDim.x+threadIdx.x;
    if(id<(m*n))
    {
        long int i=id/n;
        long int j=id%n;
        double temp=floor((0.5*(double)d_red[i * n + j])+floor(sqrt((double)d_green[i * n + j]))+(double)d_blue[i * n + j]);
        d_T2[i * n + j]=(long int)temp;
    }
}
